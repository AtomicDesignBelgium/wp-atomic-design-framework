...
class ADF_InternalNotes {
    ...
    function admin_page() {
        echo '<div class="wrap"><h1>Internal Notes</h1>';
        if (current_user_can('edit_pages')) {
        ...
        $sf = isset($_GET['adf_so']) ? sanitize_text_field($_GET['adf_so']) : 'priority';
        echo '<div id="adfFilters" style="margin:12px 0;padding:8px;border:1px solid #eee;background:#fafafa;border-radius:6px">';
        echo '<form method="get" action="'.esc_url($_SERVER['REQUEST_URI'].'#adfFilters').'" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">';
        ...
        if (!empty($notes)) {
            ...
            foreach ($notes as $n) {
                ...
                echo '<span style="display:inline-flex;align-items:center;gap:6px"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:'.$pcolor.'"></span><span style="color:#3498db">'.esc_html($n['title']).'</span></span>';
                ...
                if (!empty($comments)) {
                    ...
                    foreach ($comments as $c) {
                        ...
                        echo '<div style="font-weight:600"><span style="display:inline-block;width:12px;height:12px;background:#3498db;border-radius:2px;margin-right:6px;vertical-align:middle"></span>'.esc_html($cauthor).' · '.esc_html($time).'</div>';
                        echo '<div style="margin-top:4px">'.wp_kses_post($c['content']).'</div>';
                        $atts = [];
                ...
                if (current_user_can('edit_pages')) {
                    ...
                    echo '<textarea class="adfCommentText" data-note="'.$nid.'" placeholder="Add a reply" style="width:100%;height:40px"></textarea>';
                    echo '<div class="adf-thread-actions" style="margin-top:6px;display:flex;justify-content:flex-end;align-items:center;gap:8px">';
    ...
    function get_notes($args=[]) {
        ...
        if (!empty($args['sort'])) {
            ...
            else if ($args['sort'] === 'date') { $order = 'ORDER BY created_at ASC'; }
        }
